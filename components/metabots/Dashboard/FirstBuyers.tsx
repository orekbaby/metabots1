...
const FirstBuyers:FC<FirstBuyersProps> = ({firstBuyersData, loading, tokenAddressDynamic}) => {
 
  if (loading) {
    return <p>Loading...</p>;
  }

  if (!firstBuyersData || firstBuyersData.length === 0) {
  ...
  <TableHeader className="">
    <TableRow className="border-none outline-0">
      ...
      <TableHead className="text-center w-[50px] md:w-[80px] lg:w-[80px] font-medium text-[#E0E0E0] text-[9px] md:text-[12px] lg:text-[14px]">
       Address
      </TableHead>
      <TableHead className="text-center w-[120px] md:w-[150px] lg:w-[150px] font-medium text-[9px] md:text-[12px] lg:text-[14px]">
       Invested
  ...
  <TableBody>
    {firstBuyersData?.map((item, index) => (
      <TableRow key={index} className="border-none outline-0">
      <TableCell className=" w-[120px] md:w-[150px] lg:w-[100px] font-normal text-[9px] text-right md:text-xs lg:text-[10px] text-[#E7E7E7]">
  {formatTimestamp(item?.last_transaction_time)}
</TableCell>

        <TableCell className="text-center  pl-6 flex justify-center items-center gap-1 w-[50px] md:w-[80px] lg:w-[100px] font-normal text-[9px] md:text-xs lg:text-sm pt-5">
  <span
...
<TableCell className="text-center w-[120px] md:w-[150px] lg:w-[150px] font-normal text-[9px] md:text-sm lg:text-sm">
  {formatToDollars(item?.total_invested)}
        ...
        <TableCell className="text-center pl-8 w-[120px] md:w-[150px] lg:w-[150px] font-normal text-[9px] md:text-xs lg:text-xs  text-xs text-[#4CA244]">
        {formatToDollars(item?.realized)}
        </TableCell>
        <TableCell className="text-left w-[120px] md:w-[150px] lg:w-[150px] font-normal text-[9px] md:text-xs lg:text-xs pb-5">
  <div className="flex flex-col gap-1">
    ...
    <div className="flex justify-between items-center">
      ...
      <div className="flex items-center gap-1">
        ...
        <p className="font-normal text-[9px]">held</p>
      </div>
      {/* Right: Sold */}
      <div className="flex items-center gap-1">
        <span className="font-normal text-[9px] text-red-500">{formatNumber(item?.sold)}</span>
