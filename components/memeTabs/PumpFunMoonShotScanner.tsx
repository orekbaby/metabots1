...
import TrendingPump from './TrendingPump';
import { LatestData, TrendingData } from '@/utils/types';
import TradeSettings from '../TradeSettings';
import { BsFillExclamationCircleFill } from 'react-icons/bs';
import { useFastBuy } from "@/context/FastBuyContext";
import { fetchTrendingPumpMoon } from '@/utils/apiCalls';
interface PumpFunMoonShotScannerProps{
...
const PumpFunMoonShotScanner:FC<PumpFunMoonShotScannerProps> = ({freshMinted, moon, dex, setFreshMinted, setMoon, setDexData, }) => {
  const { isOn, toggleIsOn } = useTpSl();
  const [trendingPump, setTrendingPump] =useState<TrendingData[]>([]);
  const { fastBuyValue, setFastBuyValue } = useFastBuy(); // Get context value and setter
 
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setFastBuyValue(event.target.value); // Update the value in the context
  };

  useEffect(() => {
    const fetchTrendingDetails = async () => {
      try {
        const result = await fetchTrendingPumpMoon();
        console.log("Result from fetchTrendingPumpMoon:", result); 
  
        if (result) {
         
          setTrendingPump(result?.trendingData ?? []);
        } else {
          console.warn("No data available");
        }
      } catch (error) {
        console.error("Error fetching trending pump moon:", error);
      }
    };
    fetchTrendingDetails();
  }, []);
...
>
  <TabsList className="flex h-[71px] items-center justify-between
   ...
   <div className="flex justify-start items-center gap-4">
    <TabsTrigger
      className="w-[45px] h-[29px] text-[#DBDBDC] text-[12px] md:text-sm lg:text-[13.2px] font-semibold rounded-[24px]
    ...
    <div className="flex items-center w-[153px] h-[39px]">
      <Button className="w-[98px] flex justify-center gap-1 items-center text-sm h-[39px] px-2 py-[11px] bg-[#212E40] text-white border-[#212E40] rounded-l-[8px] rounded-r-none" >
        Fast Buy
        <Image width={14} height={14} alt="sol-img" src="/solanaLogo.svg" />
      </Button>
      <input
        className="text-xs w-[55px] h-[39px] py-[8px] px-[16px] bg-[#0C141F] text-[#E7E7E7] border-[#212E40] border rounded-r-[8px] appearance-none focus:outline-none placeholder-opacity-100"
        type="text"
        placeholder="100"
        autoComplete="off"
        value={fastBuyValue}
...
<TradeSettings />
   ...
   <All freshMinted={freshMinted} moon={moon} dex={dex} setFreshMinted={setFreshMinted} setMoon={setMoon} setDexData={setDexData} />
  </TabsContent>

  <TabsContent className="w-full h-full" value="Trending">
    <TrendingPump trendingPump={trendingPump}/>
  </TabsContent>
...
export default PumpFunMoonShotScanner